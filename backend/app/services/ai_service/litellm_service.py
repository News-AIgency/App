...
class GenerateTopicsSignature(dspy.Signature):
    f"""Generate list of topics based on the topics_count InputField and the scraped news article from scraped_content InputField. No numbering, no introductory text, just topics. The result should not have any characters representing bullet points. The topics should be in the language that the language InputField specifies. Each topic should start with capital letter. The news report should be factual as well as neutral. The output is a list of topics in the topics OutputField."""

    topics_count = dspy.InputField(desc="Number of topics to generate", type=int, default=5)
    scraped_content = dspy.InputField(desc="Scraped news article", type=str)
    language = dspy.InputField(desc="Language of the news article", type=Language, default=Language.SLOVAK)
...
class GenerateTopics(dspy.Module):
    ...
    def forward(self, topics_count: int, scraped_content: str, language: Language) -> GenerateTopicsSignature:
        generated_topics = self.generate_topics(topics_count=topics_count, scraped_content=scraped_content, language=language)
...
class GenerateArticleSignature(dspy.Signature):
    ...
    headlines_count = dspy.InputField(desc="Number of headlines to generate", type=int, default=3)
...
class GenerateArticle(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, headlines_count: int, tag_count: int, language: Language) -> GenerateArticleSignature:
        generated_article = self.generate_article(scraped_content=scraped_content, selected_topic=selected_topic, headlines_count=headlines_count, tag_count=tag_count, language=language)
...
class GenerateHeadlines(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, headlines_count: int, language: Language) -> GenerateHeadlinesSignature:
        generated_headlines = self.generate_headlines(scraped_content=scraped_content, selected_topic=selected_topic, headlines_count=headlines_count, language=language)
...
class RegenerateHeadlines(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, old_headlines: str, headlines_count: int, language: Language) -> RegenerateHeadlinesSignature:
        generated_headlines = self.regenerate_headlines(scraped_content=scraped_content, selected_topic=selected_topic, old_headlines=old_headlines, headlines_count=headlines_count, language=language)
        return generated_headlines
...
class GenerateEngagingText(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, current_headline: str, language: Language) -> GenerateEngagingTextSignature:
        generated_engaging_text = self.generate_engaging_text(scraped_content=scraped_content, selected_topic=selected_topic, current_headline=current_headline, language=language)
...
class RegenerateEngagingText(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, old_engaging_text: str, current_headline: str, language: Language) -> RegenerateEngagingTextSignature:
        generated_engaging_text = self.regenerate_engaging_text(scraped_content=scraped_content, selected_topic=selected_topic, old_engaging_text=old_engaging_text, current_headline=current_headline, language=language)
        return generated_engaging_text
...
class GeneratePerex(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, current_headline: str, language: Language) -> GeneratePerexSignature:
        generated_perex = self.generate_perex(scraped_content=scraped_content, selected_topic=selected_topic, current_headline=current_headline, language=language)
...
class RegeneratePerex(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, old_perex: str, current_headline: str, language: Language) -> RegeneratePerexSignature:
        generated_perex = self.regenerate_perex(scraped_content=scraped_content, selected_topic=selected_topic, old_perex=old_perex, current_headline=current_headline, language=language)
        return generated_perex
...
class GenerateArticleBody(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, current_headline: str, language: Language) -> GenerateArticleBodySignature:
        generated_article = self.generate_article_body(scraped_content=scraped_content, selected_topic=selected_topic, current_headline=current_headline, language=language)
        return generated_article
...
class RegenerateArticleBody(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, old_article: str, current_headline: str, language: Language) -> RegenerateArticleBodySignature:
        generated_article = self.regenerate_article_body(scraped_content=scraped_content, selected_topic=selected_topic, old_article=old_article, current_headline=current_headline, language=language)
        return generated_article
...
class GenerateTags(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, current_headline: str, current_article: str, tag_count: int, language: Language) -> GenerateTagsSignature:
        generated_tags = self.generate_tags(scraped_content=scraped_content, selected_topic=selected_topic, current_headline=current_headline, current_article=current_article, tag_count=tag_count, language=language)
...
class RegenerateTags(dspy.Module):
    ...
    def forward(self, scraped_content: str, selected_topic: str, old_tags: list[str], current_headline: str, current_article: str, tag_count: int, language: Language) -> RegenerateTagsSignature:
        generated_tags = self.regenerate_tags(scraped_content=scraped_content, selected_topic=selected_topic, old_tags=old_tags, current_headline=current_headline, current_article=current_article, tag_count=tag_count, language=language)
        return generated_tags
...
class LiteLLMService:
    ...
    async def generate_topics(
        ...
        lm = dspy.LM('openai/gpt-4o-mini', api_key=settings.LITE_LLM_KEY, base_url="http://147.175.151.44/")
        ...
        generated_topics = await generate_topics_program(topics_count=topics_count, scraped_content=scraped_content, language=language)
    ...
    async def generate_article(
        ...
        generated_article = await generate_article_program(scraped_content=scraped_content, selected_topic=selected_topic, headlines_count=headlines_count, tag_count=tag_count, language=language)
    ...
    async def generate_headlines(
        ...
        generated_headlines = await generate_headlines_program(scraped_content=scraped_content, selected_topic=selected_topic, headlines_count=headlines_count, language=language)
    ...
    async def regenerate_headlines(
        ...
        generated_headlines = await regenerate_headlines_program(scraped_content=scraped_content, selected_topic=selected_topic, old_headlines=old_headlines, headlines_count=headlines_count, language=language)
    ...
    async def generate_engaging_text(
        ...
        generated_engaging_text = await generate_engaging_text_program(scraped_content=scraped_content, selected_topic=selected_topic, current_headline=current_headline, language=language)
        
        return EngagingTextResponse.model_validate_json(generated_engaging_text.engaging_text)
    ...
    async def regenerate_engaging_text(
        ...
        regenerate_engaging_text_program = dspy.asyncify(regenerate_engaging_text_program)
        generated_engaging_text = await regenerate_engaging_text_program(scraped_content=scraped_content, selected_topic=selected_topic, old_engaging_text=old_engaging_text, current_headline=current_headline, language=language)
    ...
    async def generate_perex(
        ...
        generated_perex = await generate_perex_program(scraped_content=scraped_content, selected_topic=selected_topic, current_headline=current_headline, language=language)
    ...
    async def regenerate_perex(
        ...
        generated_perex = await regenerate_perex_program(scraped_content=scraped_content, selected_topic=selected_topic, old_perex=old_perex, current_headline=current_headline, language=language)
    ...
    async def generate_article_body(
        ...
        lm = dspy.LM('openai/o1_mini', api_key=settings.LITE_LLM_KEY, base_url="http://147.175.151.44/")
        dspy.settings.configure(lm=lm, async_max_workers=8)
        ...
        generated_article = await generate_article_body_program(scraped_content=scraped_content, selected_topic=selected_topic, current_headline=current_headline, language=language)
    ...
    async def regenerate_article_body(
        ...
        generated_article = await regenerate_article_body_program(scraped_content=scraped_content, selected_topic=selected_topic, old_article=old_article, current_headline=current_headline, language=language)
    ...
    async def generate_tags(
        ...
        generated_tags = await generate_tags_program(scraped_content=scraped_content, selected_topic=selected_topic, current_headline=current_headline, current_article=current_article, tag_count=tag_count, language=language)
    ...
    async def regenerate_tags(
        ...
        generated_tags = await regenerate_tags_program(scraped_content=scraped_content, selected_topic=selected_topic, old_tags=old_tags, current_headline=current_headline, current_article=current_article, tag_count=tag_count, language=language)
